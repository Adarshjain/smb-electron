...
export function CrudCompany({company, label, onSave}: CrudCompanyProps) {
    ...
    const onSubmit = useCallback(async (data: FormData) => {
        ...
        try {
            ...
            if (data.is_default) {
                await window.api.db.query("UPDATE companies SET is_default = 0", undefined, true);
            }
                                ...
                                <FieldSet>
                                    <FieldGroup data-slot="checkbox-group">
                                        <Field orientation="horizontal" className="flex items-start">
                                            <Checkbox id="is_default" disabled={!!company?.is_default} name={field.name} checked={field.value} onCheckedChange={field.onChange}/>
                                            <div className="grid">
